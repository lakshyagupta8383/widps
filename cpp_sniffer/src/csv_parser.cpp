...
string CsvParser::trim(const string &s)
{
    ...
    while (a < s.size() && isspace((unsigned char)s[a]))
        ++a; // increments the value of if s[a] is empty space to eliminate spaces before the line starts
    size_t b = s.size();
    while (b > a && isspace((unsigned char)s[b - 1]))
        --b;                   // decrements the value of if s[b] is empty space to eliminate spaces after the line ends
...
vector<string> CsvParser::splitExactlyN(const string &s, size_t n)
{
    ...
    if (n == 0)
        return out;     // returning empty vector if n is 0
    ...
    {
        if (!getline(ss, field, ','))
            field.clear();          // uses getline to read characters from the stringstream ss into the field string until a comma delimiter is found
...
{
    if (m.size() != 17)
        return false; // 6*2=12 + 5 colons=12
    for (size_t i = 0; i < m.size(); ++i)
    {
        if ((i % 3) == 2)
        { // every third element is a colon
            if (m[i] != ':')
                return false;
        }
        else
        {
            if (!isxdigit((unsigned char)m[i]))
                return false; // check for hexadecimal value
        }
    }
    return true;
}
...
bool CsvParser::parseTimestamp(const string &s, long &ts)
{
    if (s.empty())
    {
        ts = time(nullptr);
        return false;
    } // if stirng is empty then ts is null and func returns 0
    ...
    if (iss.fail())
    {
        ts = time(nullptr);
        return false;
    } // again if ts the ts is not successfully parsed then ts is null and func returns 0
...
void CsvParser::setHeader(const string &line)
{
    ...
    for (auto &ch : firstLower)
        ch = (char)tolower((unsigned char)ch);
    // acc to the first element the type of section is decided
    if (firstLower.find("bssid") != string::npos)
        currentSection = AP_SECTION; // if first element is bssid-> the section is an AP
    else if (firstLower.find("station mac") != string::npos)
        currentSection = STATION_SECTION; // if first element is station mac> the section is an STATION
    else
        currentSection = NONE;
}
...
bool CsvParser::parseAp(const vector<string> &p, ProbeEntry &out)
{
    ...
    auto idx = [&](const string &name) -> int
    {
        ...
        if (!colIndex.count(n))
            return -1;         // if n is not found in the map then the lambda returns -1
    ...
    if (iBssid < 0 || iBssid >= (int)p.size())
        return false;       // checking wheather the idx of bssid is >=0 and < the length of the p/parts array
    string mac = p[iBssid]; // obtianing mac via ibssid idx
    if (!macValid(mac))
        return false; // validating the mac address
    ...
    { // checks that iPower is within bounds
        try
        {
            out.signal = stoi(p[iPower]);
        }
        catch (...)
        {
            out.signal = 0;
        } // if the stoi() method fails for some reason then the val 0 is set in the probeEntry
    }
    else
        out.signal = 0;      // if the idx is out of bounds
    long ts = time(nullptr); // initializing the ts with the current time val
    if (iLast >= 0 && iLast < (int)p.size())
        parseTimestamp(p[iLast], ts); // if ts idx is within bounds then the it is parsed otherwise the current time val will be used
...
bool CsvParser::parseStation(const vector<string> &p, ProbeEntry &out)
{
    ...
    if (iStation < 0 || iStation >= (int)p.size())
        return false;
...
bool CsvParser::parseLine(const string &line, ProbeEntry &out)
{
    ...
    string low = s;
    for (auto &c : low)
        c = (char)tolower((unsigned char)c); // creating lowercase copy for checking header
    ...
    if (currentSection == NONE || headerCols.empty())
        return false;
    ...
    if (currentSection == AP_SECTION)
        return parseAp(parts, out);
    if (currentSection == STATION_SECTION)
        return parseStation(parts, out);
