...
int main(int argc, char** argv){immediately
    ...
    auto openFile=[&](bool seekEnd)->bool { 
        if (file.is_open()) file.close(); // closes the old file (defind above ifstream file)
    ...
    if (!openFile(true)) { // checks if the openFile returns true and the the file is open
    ...
    while (g_running.load()) { // till no signal is sent 
        ++loopCounter;

        if (!file.good()) { // checks if the file is in "good state" i.e there are no errors
            if (!openFile(true)) {
        ...
        if (endPos > lastPos) {  //reading the new lines
            file.clear();
            file.seekg(lastPos);
            string line;
            while (getline(file, line)) { 
                ...
                ProbeEntry entry;
                if (!parser.parseLine(line, entry)) { // parse the csv line into a probe struct
                ...
                if (!ok) { // incase of unsuccessful conversion, the struct is pushed into the retryQueue
        ...
        if (!retryQueue.empty()) {
            for (auto it = retryQueue.begin(); it != retryQueue.end(); ) {
                ...
                if (!g_running.load()) break; // incase of termination 
            }
            if (!retryQueue.empty()) {
